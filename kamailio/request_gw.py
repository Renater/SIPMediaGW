...
class RequestGw:
    ...
    def handler(self, msg, args):
        ...
        launcherAddrList = (self.config['mediagw']['launcherAddrList'].
        ...
        if msg.Type == 'SIP_REQUEST':
            if (msg.Method == 'INVITE' and (msg.RURI).find(self.config['mediagw']['sipUaNamePart']) == -1:
                ...
                for launcherAddr in launcherAddrList:
                    Logger.LM_ERR('Launcher addr %s\n' % launcherAddr )
                    reqUrl = "https://" + launcherAddr + "/sipmediagw"
                    uaNamePrefix = launcherAddr.replace(".", "")
                    reqUrl += ("?room=%s&from=%s&prefix=%s" %(room, fromUri, uaNamePrefix))
