class BBBESR extends UIHelper {
    ...
    async preJoin() {
        if (window.location.href.includes('classe-virtuelle.numerique-esr.fr')) {
            try{
                const classroomBtn = await this.waitForElement(
                    'button.c__button--primary[aria-label="Click here to access classroom"], button.c__button--primary',
                    { visible: true, clickable: true }
                );
                if (classroomBtn) {
                    classroomBtn.click();
                }
                const preJoinRoomBtn = await this.waitForElement(
                    'button.c__button--primary[aria-label="Click here to access classroom"], button.c__button--primary',
                    { visible: true, clickable: true },
                    1000
                );
    ...
    async join() {
        try {
            let consentCheck;
            try {
                consentCheck = await this.waitForElement(
                    '#consentCheck, label.c__checkbox input[type="checkbox"], .c__checkbox__wrapper input[type="checkbox"]',
                    { visible: true },
                    1000
                );
            } catch (e) {
                console.error('[✗] Consent check not found:', e);
            }
            if (consentCheck) {
                consentCheck.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true }));
            }
            ...
            try {
                joinButton = await this.waitForElement(
                                            "button[type='submit'], button.c__button--primary",
                                            { clickable: true }
                                        );
            } catch (e) {
    ...
    async startAudio() {
        console.log('[INFO] Enabling microphone...');
            let micDetails;
            try {
                micDetails = await this.waitForElement("[data-test='microphoneBtn']", { visible: true });
            } catch (e) {
                console.error('[✗] Microphone button not found:', e);
                return;
            }
    ...
    async slideShot() {
        try {
            const selector = "#screenshareVideo";
...
window.BBBESR = BBBESR;
window.Browsing = BBBESR;
