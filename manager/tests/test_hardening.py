"""
The request path under load and under a hostile client:

- no blocking call on the event loop from an `async def` route;
- the password change is throttled like the sign-in;
- a malformed sign-in body is a failed attempt, not a 500;
- a year outside 1970-2100 is a 400, not a 500;
- every connection carries a statement timeout;
- the browser never receives the proxyAPI's internal address.
...
def test_async_routes_never_block_the_event_loop():
    """on the loop, a slow database froze the park view and the sampler with it."""
...
def test_the_password_change_is_throttled(alice):
    """failures were counted there but never read, so guessing went on unbounded."""
...
def test_a_malformed_sign_in_is_a_failed_attempt(accounts, body):
    """these answered 500 and did not count against the throttle."""
...
def test_years_out_of_range_are_rejected(period):
    """date(10000, 1, 1) raised a ValueError, which answered 500."""
...
def test_every_connection_has_a_statement_timeout():
    """a query that never ends held a pooled connection for good."""
    import db
...
def test_an_unreachable_proxy_keeps_its_address(monkeypatch):
    """the 502 detail carried the proxyAPI URL to the browser; the log keeps the cause."""
    monkeypatch.setitem(proxyapi.source("proxyapi"), "base_url", "http://127.0.0.1:9")
    with pytest.raises(HTTPException) as error:
        asyncio.run(park.gateways())
